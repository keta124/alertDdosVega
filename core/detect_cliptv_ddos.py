...
def alert_cliptv(level,state):
    try:
        search_by_dst = "alert.signature:\"VEGA DOS TCP TRACK BY DST CLIPVN LEVEL "+str(level)+"\""
        ...
        if level ==3 :
            ...
            if state[1] == 0 and len(list_server_attacked)>0 :
                content_email= "Server critical cliptv attack - Level 3 \n\n"+'\r\n'.join(list_server_attacked)
        ...
        if level ==2 :
            ...
            # Send Email
            if state[1] == 0 and len(list_server_attacked)>0 :
                content_email= "Server warning cliptv attack - Level 2 \n\n"+'\r\n'.join(list_server_attacked)
                TO = ['keta124@gmail.com','sontn@vega.com.vn']
                email =Email(TO,content_email)
                email.send_gmail()
                state[1] =1
        ...
        if level ==1 :
            ...
            if state[2] == 0 and len(list_server_attacked)>0:
                content_email= "Server maybe cliptv attack - Level 1 \n\n"+'\r\n'.join(list_server_attacked)
                TO = ['keta124@gmail.com','sontn@vega.com.vn']
                email =Email(TO,content_email)
                email.send_gmail()
                state[2] =1
